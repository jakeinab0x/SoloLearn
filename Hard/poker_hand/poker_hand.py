...
# hand = "2H 5C 7D AS JD".split() # High Card
...
def calc_high_card(hand_values, hand_suits, straight):
    # if there are no duplicate values or suits and no straight
    # (This is also checked in calc_hand func as we check high card last, but still best to have this check here too)
    if set(hand_values) and set(hand_suits) != 1:
        if not straight:
            return True

high_card = calc_high_card(hand_values, hand_suits, straight)
